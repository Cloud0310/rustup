...
"#;
    ...
    async fn category_mode_uses_rustup_homes_for_path_setup() {
        ...
        let mut cmd = cx
            .config
            .cmd("rustup-init", ["-y", "--default-toolchain", "nightly"]);
        ...
        assert!(env_contents.contains(&format!(r#"*:"{}":*)"#, bin_home.display())));
        assert!(env_contents.contains(&format!(r#"export PATH="{}:$PATH""#, bin_home.display())));

        let echo_path = |path: &OsStr| {
            cx.config
                .cmd("cargo", ["--echo-env", "PATH"])
        ...
        let output = echo_path(OsStr::new("/usr/bin"));
        assert!(output.status.success());
        assert_eq!(
            env::split_paths(String::from_utf8(output.stderr).unwrap().trim_end()).next(),
            Some(bin_home.clone())
        );

        let existing_path = env::join_paths([PathBuf::from("/usr/bin"), bin_home.clone()]).unwrap();
        let output = echo_path(&existing_path);
        assert!(output.status.success());
        assert_eq!(
